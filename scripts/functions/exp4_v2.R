...
optimise_nbhds <- function(dph) {
  ...
  for(destRow in 1:nrow(destList)){
    ...
    error_counter <- 0
    num_dests <- 0
    while((get_unsrvd_pop(pxlsInitial,destCode) > pop*(1-destList$coverage[destRow]))){ # loop until all are served AND we have destinations to use
...
outputSqlite <- paste0(output_deci_dir,"/pxls.sqlite")
